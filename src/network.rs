...
impl SpikingNetwork {
    ...
    pub fn apply_reinforcement(&mut self, is_success: bool) {
        ...
        println!(
            "\n📢 [ТРАССИРОВКА КРИТИКА]: Вызов подкрепления (Успех = {}). Системный тик: {}",
            is_success, current_t
        );
        ...
        println!(
            "📢 [ТРАССИРОВКА КРИТИКА]: Всего синапсов изменено: {}",
            reinforced_count
        );
    }
    ...
    pub fn sleep_and_prune(&mut self) {
        ...
        for source_id in 0..self.memory.adj_list.len() {
            ...
            for synapse in links.iter_mut() {
                ...
                println!(
                    "    💤 СОН: ID {} -> ID {} | Вес до сна: {:.2} -> После сна: {:.2}",
        ...
        for id in 0..self.memory.neurons.len() {
            if let Some(neuron) = self.memory.neurons.get_mut(id) {
                if neuron.neuron_type == NeuronType::Hidden
                    && !neuron_activity_counter.contains_key(&(id as u64))
                {
        ...
        println!(
            "🌙 [ТРАССИРОВКА СНА]: Синапсов выжжено: {}, Мета-нейронов заморожено: {}",
            pruned_synapses, removed_count
        );
    }
    ...
    pub fn tick(&mut self) {
        ...
        if !self.previous_tick_spikes.is_empty() || !current_tick_spikes.is_empty() {
            println!(
                "⏱️ [ТИК {}]: Активные спайковые вспышки: {:?}",
                self.current_tick, current_tick_spikes
            );
        }

        // =================================================================
        // КАНОНИЧНАЯ СТДП-ПЛАСТИЧНОСТЬ С ЗАЩИТОЙ ОТ КОМБИНАТОРНОГО ВЗРЫВА
        // =================================================================
        for &old_id in &self.previous_tick_spikes {
            for &new_id in &current_tick_spikes {
                if old_id != new_id {
                    // АРХИТЕКТУРНЫЙ ФИЛЬТР: Проверяем типы нейронов в ОЗУ
                    let is_old_sensor =
                        self.memory.neurons.get(old_id as usize).map_or(false, |n| {
                            n.neuron_type == crate::models::NeuronType::Sensor
                        });

                    let is_new_sensor =
                        self.memory.neurons.get(new_id as usize).map_or(false, |n| {
                            n.neuron_type == crate::models::NeuronType::Sensor
                        });

                    // Склеивать в чанки и регистрировать совпадения разрешено ТОЛЬКО
                    // для базовых слов словаря! Мета-абстракции не имеют права скрещиваться.
                    if is_old_sensor && is_new_sensor {
                        ...
                        *count += 1;
                        ...
                        let current_weight = self.memory.get_synapse_weight(old_id, new_id);
                        let next_weight = if current_weight < 0.1 {
                            0.5
                        } else {
                            (current_weight + 0.15).min(1.2)
                        };
                        self.memory
                            .set_synapse(old_id, new_id, next_weight, self.current_tick);

                        if *count == cfg.coincidence_threshold {
                            let meta_neuron_id = self.memory.create_meta_chunk(old_id, new_id);

                            println!("✨ [МЕТА-ЭВОЛЮЦИЯ]: Преодолен порог совпадений! Рождение чанка ID {} -> ID {} с новым ID: {}", 
                                old_id, new_id, meta_neuron_id);

                            self.memory
                                .set_synapse(old_id, meta_neuron_id, 0.6, self.current_tick);
                            self.memory
                                .set_synapse(new_id, meta_neuron_id, 0.6, self.current_tick);
                        }
                    } else {
                        // Если в цепочке участвует мета-нейрон, мы просто плавно
                        // транслируем базовый заряд дальше по графу, не создавая новых вложенных чанков-монстров!
                        let current_weight = self.memory.get_synapse_weight(old_id, new_id);
                        if current_weight > 0.1 {
                            self.memory.set_synapse(
                                old_id,
                                new_id,
    ...
    pub fn generate_autonomous_mutation(
        ...
        let mut current_id = match self.memory.lookup_token_id(start_token) {
            ...
            None => {
                println!(
                    "    ❌ КРИТИЧЕСКАЯ ТРАССИРОВКА: Стартовый токен '{}' отсутствует в словаре!",
                    start_token
                );
        ...
        for step in 0..10 {
            ...
            if let Some(links) = self.memory.adj_list.get(current_id as usize) {
                ...
                for synapse in links.iter() {
                    ...
                    // Если путь уже посещался, мы накладываем на него
                ...
                println!(
                    "  📍 Шаг {}: Узел ID {} физически отсутствует в adj_list!",
                    step, current_id
                );
            }
